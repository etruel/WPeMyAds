...
function wpemyads_load_textdomain() {
		// Set filter for language directory
		$lang_dir = plugin_dir_path( __FILE__ ).'lang/';
		$lang_dir = apply_filters( 'wpemyads_languages_directory', $lang_dir );

		// Traditional WordPress plugin locale filter
		$locale = apply_filters( 'plugin_locale', get_locale(), 'myads' );
		$mofile = sprintf( '%1$s-%2$s.mo', 'myads', $locale );

		// Setup paths to current locale file
		$mofile_local   = $lang_dir . $mofile;
		$mofile_global  = WP_LANG_DIR . '/myads/' . $mofile;
		 ...
		 * @since 1.6.2
		 */
		$mofile_global2  = WP_LANG_DIR . '/plugins/myads/' . $mofile;

		if( file_exists( $mofile_global ) ) {
			// Look in global /wp-content/languages/myads/ folder
			load_textdomain( 'myads', $mofile_global );
		} elseif( file_exists( $mofile_global2 ) ) {
			// Look in global /wp-content/languages/plugins/myads/ folder
			load_textdomain( 'myads', $mofile_global2 );
		} elseif( file_exists( $mofile_local ) ) {
			// Look in local /wp-content/plugins/myads/languages/ folder
			load_textdomain( 'myads', $mofile_local );
		} else {
			// Load the default language files
			load_plugin_textdomain( 'myads', false, $lang_dir );
		}
	}
...
function etruel_AdServe_AddPages() {
	...
	# add submenu
	$mypage = add_menu_page(__('My Ads','myads'), __('My Ads','myads'), 'publish_posts',  'admanage', 'etruel_AdServe_Manage',plugin_dir_url( __FILE__ ) . 'files/Ad20.png');
	add_submenu_page('admanage', __('Ads Manager','myads'), __('Ads Manager','myads'), 'publish_posts', 'admanage', 'etruel_AdServe_Manage');
...
function AdServe_admin_head() { 
	...
	<script type="text/javascript">
		...
		function loadData(id, title, scriptcode, url, src, email, keywords, width, height, user,dateto, dateuntil,checkingdate){
			jQuery('#tableform .wrap h2').text('<?php echo _e("Edit Ad","myads"); ?>'); 
			document.getElementById('buttonsave').value='<?php echo _e("Save","myads"); ?>'; 
			...
			if(scriptcode==1){
				jQuery(".isscript_src").html('<textarea id="src" name="src" cols="75" rows=5></textarea>');
			}else{
		...
		function changeval(newval){
			...
			if(newval==1){
				...
				jQuery(".isscript").fadeOut(0);
				jQuery("#uploadf").hide();
				jQuery(".isscript_src").html('<textarea id="src" name="src" cols="75" rows=5>'+document.getElementById('src').value +'</textarea>');
			}else{
				sc.prop("checked", false);
				jQuery(".isscript").fadeIn(0);
				jQuery("#uploadf").show();
		...
		jQuery(document).ready( function($j) {
			...
			$j("#closefrm").click(function() {
				...
				jQuery('#tableform').fadeOut(function(){
					jQuery("#myads_mask").fadeOut();
					jQuery("#checkingdate").attr('checked',false);
			...
			$j("#reset").click(function() {   //New ad
				...
				jQuery('#myads_mask').fadeIn(0,function(){
					$j('#adform')[0].reset();
					document.getElementById('buttonsave').value='<?php echo _e("Add","myads"); ?>'; 
					$j('#tableform .wrap h2').text('<?php echo _e("Add ad","myads");  ?>'); 
			...
			jQuery(document).on('click','.adedit',function(){
				jQuery('#myads_mask').show(0);
			});
...
function etruel_AdServe_Manage() {
	...
	<?php
	print "<div id='table-content'><table class='widefat AdsTable'><thead><tr><th scope='col'>".__('Site','myads')."</th><th scope='col'>".__('Zones','myads')."</th><th scope='col'>".__('Active','myads')."</th><th scope='col'>".__('Impressions','myads')."</th><th scope='col'>".__('Clicks','myads')."</th><th scope='col'>".__('Ratio','myads')."</th><th scope='col'>".__('Credits','myads')."</th>";
	if(etruel_check_user_role('administrator') ) print "<th scope='col'>".__('User','myads')."</th>";
	print "<th scope='col'>".__('Actions','myads')."</th>
	<th>A/D</th>
	</tr></thead>";
	print "<tbody id='the-list'>";
	...
	//	$vardateuntil = date_i18n('d/m/Y H:m:s',strtotime($rk->dateuntil));  
		$vardateto = date_i18n('d-m-Y H:i:s',strtotime($rk->dateto)); 
		$vardateuntil = date_i18n('d-m-Y H:i:s',strtotime($rk->dateuntil));
...
#		print "<td>".$rk->weight."</td>\n";
	...
	<td>
		...
		<center><h2><?php echo (!$first)?_e("Add","myads"):_e("Edit","myads");  ?> <?php echo _e('Ad','myads'); ?></h2></center>
		...
		<table>
			<tr>
				...
				<td><span style="color:#1ABC9C; font-size:14px; font-weight:bold;"><?php echo _e('Ad Details','myads'); ?></span></td>
			</tr>
			<tr><td><?php  echo _e('Title:','myads'); ?></td><td><input type="text" name="title" id="title" value="" size="60"></td></tr>
			...
			<tr><td>*Src</td><td><span class="isscript_src"><input type="text" id="src" name="src" value="" size="60"></span><input type='button' class='button' id='uploadf' value='<?php echo _e("Upload","myads"); ?>'>
				<span>Ej: http://domain.com/image.jpg|.png|.gif|.swf</span></td></tr>
			<!--properties-->
			<tr>
				<td><span class="number" style="font-size:14px;">2</span></td>
				<td style="color:#1ABC9C; font-size:14px; font-weight:bold;"><?php echo _e('Ad properties','myads'); ?></td>
			</tr>
			<tr><td><?php echo _e('Zones','myads'); ?> *</td><td><input type="text" name="keywords" id="keywords" value="" size="30"> <?php
					$zones = array();
					...
					echo $doSelect;
			?></td></tr>
			<tr><td><?php echo _e('Height','myads'); ?> *</td><td><input type="text" name="width" id="width" value=""></td></tr>  			
			<tr><td><?php echo _e('Width','myads'); ?>* </td><td><input type="text" name="height" id="height" value=""></td></tr>   
			...
			<?php endif; ?>
			<tr><td><?php echo _e('Email','myads'); ?></td><td><input type="text" name="email" id="email" value="" size="40"></td></tr>
			<tr>
				<td><b><?php echo _e('Programming Date','myads'); ?>:</b></td>
				<td><input type="checkbox" value="Y" id="checkingdate" name="checkingdate"></td>
			</tr>
			<tr id="datefields_ads" style="display:none;">
				<td><?php echo _e('Date to','myads'); ?>:</td>
				<td>
					<input class="fieldate time-input" type="text" name="dateto" id="dateto">
					<span><?php echo _e('Date Until','myads'); ?></span>
			...
			<tr><td><input  class="button-primary"type="submit" name="vai" value="<?php _e('Add','myads'); ?>" id="buttonsave"></td></tr>
		</table>
	</form>
...
function etruel_AdServe_Dashboard() {
	...
	global $user_login;
	$table_name = $wpdb->prefix . "adserve";
	print "<div class='wrap'><h2>Your Ads</h2><table class='widefat tabreport'><thead><tr><th scope='col'>"._e('Site','myads')."</th><th scope='col'>"._e('Zones','myads')."</th><th scope='col'>"._e('Active','myads')."</th><th scope='col'>"._e('Impressions','myads')."</th><th scope='col'>"._e('Clicks','myads')."</th><th scope='col'>"._e('Ratio','myads')."</th><th scope='col'>"._e('Credits','myads')."</th></tr></thead>";
	print "<tbody id='the-list'>";
